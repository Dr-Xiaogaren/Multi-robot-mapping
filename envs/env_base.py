...
class BaseEnv(gym.Env):
    ...
    def load(self):
        ...
        # Get robot config
        robot_config = self.config["robots"]
        num_robots = self.config.get("n_robots")
        assert len(robot_config) == num_robots, "Check the number of robots in the config file"
        # If no robot has been imported from the scene
        if len(scene.robots) == 0:
            # Get corresponding robot class
            for each_robot in robot_config:
                robot_id = each_robot.pop("id")
                robot_name = each_robot.pop("name")
                assert robot_name in REGISTERED_ROBOTS, "Got invalid robot to instantiate: {}".format(robot_name)
                robot = REGISTERED_ROBOTS[robot_name](**each_robot)
        ...
        self.n_robots = len(scene.robots)
