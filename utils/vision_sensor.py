...
class VisionSensor(BaseSensor):
    ...
    def get_obs(self, env):
        ...
        modality_length = len(self.modalities)
        all_raw_vision_obs = []

        for i in range(0, len(raw_vision_obs), modality_length):
            single_raw_vision_obs = {mode: value for mode, value in zip(self.raw_modalities, raw_vision_obs[i:i+modality_length])}
            all_raw_vision_obs.append(single_raw_vision_obs)


        vision_obs = OrderedDict()
        if "rgb" in self.modalities:
            vision_obs["rgb"] = [self.get_rgb(single_raw_vision_obs) for single_raw_vision_obs in all_raw_vision_obs]
        if "rgb_filled" in self.modalities:
            vision_obs["rgb_filled"] = [self.get_rgb_filled(single_raw_vision_obs) for single_raw_vision_obs in all_raw_vision_obs]
        if "depth" in self.modalities:
            vision_obs["depth"] = [self.get_depth(single_raw_vision_obs) for single_raw_vision_obs in all_raw_vision_obs]
        if "pc" in self.modalities:
            vision_obs["pc"] = [self.get_pc(single_raw_vision_obs) for single_raw_vision_obs in all_raw_vision_obs]
        if "optical_flow" in self.modalities:
            vision_obs["optical_flow"] = [self.get_optical_flow(single_raw_vision_obs) for single_raw_vision_obs in all_raw_vision_obs]
        if "scene_flow" in self.modalities:
            vision_obs["scene_flow"] = [self.get_scene_flow(single_raw_vision_obs) for single_raw_vision_obs in all_raw_vision_obs]
        if "normal" in self.modalities:
            vision_obs["normal"] = [self.get_normal(single_raw_vision_obs) for single_raw_vision_obs in all_raw_vision_obs]
        if "seg" in self.modalities:
            vision_obs["seg"] = [self.get_seg(single_raw_vision_obs) for single_raw_vision_obs in all_raw_vision_obs]
        if "ins_seg" in self.modalities:
            vision_obs["ins_seg"] = [self.get_ins_seg(single_raw_vision_obs) for single_raw_vision_obs in all_raw_vision_obs]
        if "highlight" in self.modalities:
            vision_obs["highlight"] = [self.get_highlight(single_raw_vision_obs) for single_raw_vision_obs in all_raw_vision_obs]

        for key in vision_obs.keys():
            vision_obs[key] = np.stack(vision_obs[key], axis=0)
        return vision_obs
